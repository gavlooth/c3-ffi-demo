...
void run_store_barrier_merge_tests(void) {
    ...
    TEST("store barrier chooses merge large");
    {
        ...
        /* Allocate many arena objects in src (> threshold, >64 bytes to avoid inline) */
        /* Create 100 pairs in src region to exceed 4KB threshold */
        Obj* large_value = mk_cell_region(src, mk_int_region(src, 0), mk_int_region(src, 0));
        for (int i = 0; i < 100; i++) {
        ...
        /* Verify src has no inline allocations (needed for merge) */
