...
void run_store_barrier_merge_tests(void) {
    ...
    TEST("store barrier chooses transmigrate small");
    {
        ...
        Obj* container = mk_pair_region(dst, mk_int_region(dst, 0), mk_int_region(dst, 0));
    ...
    TEST("store barrier chooses merge large");
    {
        Region* src = region_create();
        Region* dst = region_create();
        omni_region_set_lifetime_rank(dst, 0);
        ...
        /* Allocate large nested pair in src (> threshold, >64 bytes to avoid inline) */
        /* Create a deep pair structure with large values */
        char* large_data = (char*)region_alloc(src, 5000);
        ASSERT(large_data != NULL);
        strcpy(large_data, "large value");
        
        /* Create value in src to store */
        Obj* large_value = mk_pair_region(src, 
                                          mk_int_region(src, 999),
                                          mk_int_region(src, 888));
        
        /* Verify large_value is in src region */
        ASSERT(omni_obj_region(large_value) == src);
        
        /* Create container in dst */
        Obj* container = mk_pair_region(dst, mk_int_region(dst, 0), mk_int_region(dst, 0));
        
        /* Store large_value into container (older <- younger): should attempt merge */
        /* Since src is arena-only and large, merge should succeed */
        Obj* result = omni_store_repair(container, &container->a, large_value);
        
        ASSERT(result != NULL);
        /* After successful merge, value should be same (arena transfer, not copy) */
        ASSERT(result == large_value);
        
        /* Verify data still accessible after src exit */
        /* After merge, arena chunks transferred to dst, so data still valid */
        region_exit(src);
        region_destroy_if_dead(src);
        ASSERT(strcmp(large_data, "large value") == 0);
